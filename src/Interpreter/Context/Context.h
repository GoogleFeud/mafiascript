...
public:
    ...
    MS_VALUE executeAST(AST_NODE *node, Enviourment *env)
    {
        ...
        case AST_Types::MS_ACCESSOR: {
            ...
            MS_VALUE start = env->get(access->start);
        ...
        case AST_Types::MS_CALL:
        {
            ...
            else if (functionObj.index() == MS_Types::C_FUNCTION)
            {
                ...
                throw std::runtime_error("Cannot call a " + typeToString(functionObj.index()));
        };
        ...
        case AST_Types::MS_AND:
        {
            ...
            if (!isFalsey(left))
                return this->executeAST(_and->right, env);
            return false;
        };
        case AST_Types::MS_OR:
        {
            ...
            if (isFalsey(left))
                return this->executeAST(_or->right, env);
            return left;
        };
        ...
        case AST_Types::MS_IF:
        {
            ...
            if (!isFalsey(cond))
            {
                if (ms_if->ifTrue->index() != AST_Types::MS_BLOCK)
                    return this->executeAST(ms_if->ifTrue, env);
            ...
            else if (ms_if->ifFalse != NULL)
            {
                if (ms_if->ifFalse->index() != AST_Types::MS_BLOCK)
                    return this->executeAST(ms_if->ifFalse, env);
                Enviourment *newEnv = env->extend();
        ...
        case AST_Types::MS_BLOCK: {
            ...
            for (AST_NODE *blockNode : block->nodes) {
                ...
                if (res.index() != MS_Types::T_NULL && (blockNode->index() == AST_Types::MS_IF || blockNode->index() == AST_Types::MS_LOOP))
                    return res;
            };
            return MS_VALUE{nullptr};
        };
